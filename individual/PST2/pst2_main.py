...
def print_student_card(student_id):
    ...
    if student_to_print:
        ...
        with open(filename, 'w') as f:
            ...
            #.join() concatenates string members of an iterable (e.g. a list) to form a single string with separator specified before the.join()
            #.get(a,b) is used to retrieve the value of a key in a dictionary. 
            #The parameter a is the key who's value is to be returned and if a does not exist then the default b is returned
            #This prevents errors from occuring in the case a does not exist in the dictionary
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nSelect updates to be made: ")
            print("1. update name only")
            print("2. update speciality only")
            print("3. update name and speciality")

            selection=input("Select choice: ")
            if selection=="1":
                teacher_name=input("Teacher name: ")
            elif selection=="2":
                teacher_speciality=input("Teacher speciality: ")
            else:
                teacher_name=input("Teacher name: ")
                teacher_speciality=input("Teacher speciality: ")

            update_teacher(teacher_id,teacher_name,teacher_speciality)
            made_change = True
        elif choice == '4':
